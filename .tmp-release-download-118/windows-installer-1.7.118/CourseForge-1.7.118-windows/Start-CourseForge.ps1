...
$integrityScript = Join-Path $scriptDir "Test-CourseForge-Integrity.ps1"
$integrityStatusPath = Join-Path $scriptDir "integrity-status.json"
...
$bootStatusPath = Join-Path $scriptDir "boot-status.json"
...
try {
  ...
  foreach ($candidateLog in @($launcherLog, $serverStdoutLog, $serverStderrLog)) {
  ...
  $fallbackRoot = if ([string]::IsNullOrWhiteSpace($env:TEMP)) { $scriptDir } else { $env:TEMP }
  $logDir = Join-Path $fallbackRoot "CourseForge-launcher"
  try {
    New-Item -ItemType Directory -Path $logDir -Force -ErrorAction Stop | Out-Null
    $launcherLog = Join-Path $logDir "launcher.log"
    $serverStdoutLog = Join-Path $logDir "server-stdout.log"
...
function Write-BootStatus {
  ...
  $safePercent = [Math]::Min(100, [Math]::Max(0, $ProgressPercent))
  $payload = [ordered]@{
    step = if ([string]::IsNullOrWhiteSpace($Step)) { $null } else { $Step }
  ...
  try {
    Set-Content -Path $bootStatusPath -Value $payload -Encoding ASCII
  }
  catch {
    # Best effort status file.
  }
}
...
Write-BootStatus -Step "integrity-check" -Message "Validating package integrity" -ProgressPercent 28
if (Test-Path $integrityScript) {
  try {
    & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $integrityScript -PackageRoot $scriptDir -OutputPath $integrityStatusPath -Quiet
    $integrityExitCode = $LASTEXITCODE
    if ($integrityExitCode -eq 0) {
      Write-LauncherLog "Integrity check passed."
    }
    else {
      Write-LauncherLog "WARNING: Integrity checker returned non-zero exit code $integrityExitCode. Review $integrityStatusPath"
    }
  }
  catch {
    Write-LauncherLog "WARNING: Failed to execute integrity checker. $($_.Exception.Message)"
  }
}
else {
  Write-LauncherLog "WARNING: Integrity checker script missing at $integrityScript"
}
...
function Open-BootSplash {
  ...
  try {
    $runtimeSplashDir = Join-Path $env:TEMP "CourseForge-launcher"
    New-Item -ItemType Directory -Path $runtimeSplashDir -Force -ErrorAction Stop | Out-Null
    ...
    Write-LauncherLog "Opened startup splash at $splashPath"
    return $true
  }
  catch {
    Write-LauncherLog "WARNING: Failed to open boot splash. $($_.Exception.Message)"
    return $false
  }
}
...
try {
  Write-BootStatus -Step "initializing" -Message "Preparing startup" -ProgressPercent 5
  $activePort = $port
  $url = "http://${hostName}:$activePort"
  $openedBootSplash = Open-BootSplash -Url $url
  Write-BootStatus -Step "runtime-check" -Message "Checking runtime health" -ProgressPercent 18 -Url $url
  ...
  if (-not (Test-PortAvailable -TargetHost $hostName -Port $activePort)) {
    ...
    if (Test-CourseForgeServerReady -BaseUrl $url -TimeoutSeconds 3) {
      ...
      if (-not (Test-PortAvailable -TargetHost $hostName -Port $activePort)) {
        ...
        if ($isExpectedInstall -or -not $isCourseForgeProcess) {
          ...
          Write-BootStatus -Step "ready" -Message "CourseForge is ready" -ProgressPercent 100 -Ready $true -Url $url
          exit 0
        }
      }
    }
    ...
    if (-not (Test-PortAvailable -TargetHost $hostName -Port $activePort)) {
      ...
      $url = "http://${hostName}:$activePort"
      if ($openedBootSplash) {
        Write-BootStatus -Step "port-fallback" -Message "Switching to an available local port" -ProgressPercent 24 -Url $url
      }
    }
  }
  ...
  Write-BootStatus -Step "server-start" -Message "Starting local server" -ProgressPercent 35 -Url $url
  ...
  while ((Get-Date) -lt $deadline) {
    ...
    if (Wait-ForHttpReady -Url $url -TimeoutSeconds 2 -PollMilliseconds 250) {
      $ready = $true
      Write-BootStatus -Step "finalizing" -Message "Finalizing startup" -ProgressPercent 90 -Url $url
      break
    }
  }
  ...
  Write-BootStatus -Step "failed" -Message "Startup failed" -ProgressPercent 100 -Ready $false
  Write-Host ""
