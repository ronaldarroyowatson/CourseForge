...
- Update payload and manifest context:
  - `deployment/`
  - `manifests/`
  - `otto-update/manifests/`
- Mempalace roots in Otto repos:
  - `otto-kernel/mempalace/`
  - `otto-command-service/mempalace/`
  - `otto-update/mempalace/`
  - `otto-extensions/mempalace/`
  - `otto-protocol/mempalace/`
  - `otto-ui/mempalace/`
  - `otto-server/mempalace/`

## Recommended Next-Stage Checklist

1. Pull latest `main` in all repos before further edits.
2. Re-run repo-level sanity checks where touching runtime code:
   - `npm.cmd run test`
   - `npm.cmd run typecheck`
3. If touching release/update logic, update both:
   - `mempalace/gitops/repo-sync-index.json`
   - `mempalace/testing/testing-cycle-YYYY-MM-DD.json`
