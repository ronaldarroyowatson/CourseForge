...
```bash
npm run orchestrate:installers:wait -- --description "<release description>" --ref <branch>
```

## Validate Returned Artifacts

1. Confirm Windows and Linux jobs succeeded in GitHub Actions.
2. Confirm artifact version matches `package.json` version.
3. Confirm checksums exist for each platform artifact.
4. Confirm Windows verification logs and Linux lifecycle guardrail logs show success.
...
- Keep Linux installer validation in workflow by default; do not bypass without explicit incident note.
