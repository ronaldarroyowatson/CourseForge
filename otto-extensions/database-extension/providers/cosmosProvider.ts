...
export class CosmosProvider implements DatabaseProvider {
  readonly providerName = 'cosmos' as const;

  private readonly options: CosmosProviderOptions;
  private readonly store = new Map<CollectionName, Map<string, unknown>>();

  constructor(options: CosmosProviderOptions) {
  ...
  async testConnection(): Promise<boolean> {
    if (this.options.strictApiKey && !this.options.context.apiKey) {
      throw new ProviderConnectionError(this.providerName, 'Missing COSMOS_API_KEY.');
    }
