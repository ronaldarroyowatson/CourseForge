...
function Write-UpdateState {
  param(
    ...
    [Int64]$DownloadSpeedBytesPerSecond = -1,
    [string]$ReleaseUrl = "",
    [string]$Message = "",
    [string]$LastError = "",
    [int]$FilesTotal = -1,
  ...
  $payload = [ordered]@{
    ...
    downloadSpeedBytesPerSecond = if ($DownloadSpeedBytesPerSecond -ge 0) { $DownloadSpeedBytesPerSecond } else { $null }
    ...
    filesTotal = if ($FilesTotal -ge 0) { $FilesTotal } else { $null }
    filesPlanned = if ($FilesPlanned -ge 0) { $FilesPlanned } else { $null }
    filesProcessed = if ($FilesProcessed -ge 0) { $FilesProcessed } else { $null }
    filesFailed = if ($FilesFailed -ge 0) { $FilesFailed } else { $null }
...
function Get-UpdatePlanStats {
  ...
  foreach ($sourceFile in $sourceFiles) {
    $relative = Get-RelativePath -BasePath $SourceRoot -FullPath $sourceFile.FullName
    ...
    $destinationInfo = Get-Item -LiteralPath $destinationPath -ErrorAction SilentlyContinue
    if ($null -eq $destinationInfo -or $destinationInfo.Length -ne $sourceFile.Length) {
      $planned++
      continue
    }
    ...
    try {
      ...
      # Conservative: if hashing fails, include file in planned changes.
      $planned++
    }
  }
...
function Test-ExtractedPackageContract {
  param(
    [string]$SourceRoot,
  ...
  if (-not (Test-Path -LiteralPath $packageManifestPath)) {
    return [ordered]@{
      ok = $false
      errorCode = "missing-package-manifest"
      message = "Downloaded update is missing package-manifest.json."
    }
  }
  ...
  try {
    ...
    return [ordered]@{
      ok = $false
      errorCode = "invalid-package-manifest"
      message = "Downloaded update contains an invalid package-manifest.json."
    }
  }

  $packageVersionText = [string]$packageManifest.version
  $packageVersion = ConvertTo-VersionObject $packageVersionText
  if ($null -eq $packageVersion) {
    return [ordered]@{
      ok = $false
      errorCode = "invalid-package-version"
      message = "Downloaded update package version is missing or invalid."
    }
  }

  if ($packageVersion.ToString() -ne $ExpectedVersion) {
    return [ordered]@{
      ok = $false
      errorCode = "package-version-mismatch"
      message = "Downloaded update package version '$($packageVersion.ToString())' did not match expected version '$ExpectedVersion'."
    }
  }
  ...
  if (-not (Test-Path -LiteralPath $manifestPath)) {
    return [ordered]@{
      ok = $false
      errorCode = "missing-manifest"
      message = "Downloaded update is missing manifest.json."
    }
  }
  ...
  try {
    ...
    return [ordered]@{
      ok = $false
      errorCode = "invalid-manifest"
      message = "Downloaded update contains an invalid manifest.json."
    }
  }

  $manifestVersionText = [string]$manifest.version
  $manifestVersion = ConvertTo-VersionObject $manifestVersionText
  if ($null -eq $manifestVersion) {
    return [ordered]@{
      ok = $false
      errorCode = "invalid-manifest-version"
      message = "Downloaded update manifest version is missing or invalid."
    }
  }

  if ($manifestVersion.ToString() -ne $ExpectedVersion) {
    return [ordered]@{
      ok = $false
      errorCode = "manifest-version-mismatch"
      message = "Downloaded update manifest version '$($manifestVersion.ToString())' did not match expected version '$ExpectedVersion'."
    }
  }

  $integrityScriptPath = Join-Path $SourceRoot "Test-CourseForge-Integrity.ps1"
  if (-not (Test-Path -LiteralPath $integrityScriptPath)) {
    return [ordered]@{
      ok = $false
      errorCode = "missing-integrity-script"
      message = "Downloaded update is missing Test-CourseForge-Integrity.ps1."
    }
  }

  $integrityStatusPath = Join-Path $SourceRoot "integrity-status.json"
  & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $integrityScriptPath -PackageRoot $SourceRoot -OutputPath $integrityStatusPath -Quiet | Out-Null
  $integrityExitCode = $LASTEXITCODE
  $integrityStatus = $null

  if (Test-Path -LiteralPath $integrityStatusPath) {
    try {
      $integrityStatus = Get-Content -Path $integrityStatusPath -Raw | ConvertFrom-Json
    }
    catch {
      $integrityStatus = $null
    }
  ...
  if ($integrityExitCode -ne 0) {
    $summary = ""
    if ($null -ne $integrityStatus -and $null -ne $integrityStatus.summary) {
      $summary = " tracked=$($integrityStatus.summary.trackedFiles) missing=$($integrityStatus.summary.missing) modified=$($integrityStatus.summary.modified) corrupted=$($integrityStatus.summary.corrupted) extras=$($integrityStatus.summary.extras)"
    }

    return [ordered]@{
      ok = $false
      errorCode = "integrity-check-failed"
      message = "Downloaded update failed integrity validation.$summary"
    }
  }
    ...
    try {
      ...
      while (($read = $responseStream.Read($buffer, 0, $buffer.Length)) -gt 0) {
        ...
        if ($null -ne $OnProgress) {
          ...
          if ($elapsedMs -ge 250) {
            $elapsedSec = [Math]::Max(0.001, ((Get-Date) - $downloadStartedAt).TotalSeconds)
            $speed = [Int64][Math]::Round($downloaded / $elapsedSec)
...
try {
  ...
  Download-FileWithProgress -Uri $asset.browser_download_url -OutFile $zipPath -Headers $headers -TimeoutSec 120 -OnProgress {
    param($downloadedBytes, $totalBytes, $downloadSpeedBytesPerSecond)
    $effectiveTotal = if ($assetSizeBytes -gt 0) { $assetSizeBytes } else { [Int64]$totalBytes }
    Write-UpdateState -Root $resolvedRoot -State "downloading" -Mode $mode -CurrentVersion $CurrentVersion -LatestVersion $latestVersionText -AssetName $assetName -AssetSizeBytes $effectiveTotal -BytesDownloaded ([Int64]$downloadedBytes) -DownloadSpeedBytesPerSecond ([Int64]$downloadSpeedBytesPerSecond) -ReleaseUrl $releaseUrlText -Message "Downloading update package"
  }
  ...
  $contractValidation = Test-ExtractedPackageContract -SourceRoot $sourceRoot -ExpectedVersion $latestVersion.ToString()
  if (-not $contractValidation.ok) {
    Write-UpdateLog -LogPath $logPath -Message $contractValidation.message
    Write-UpdateState -Root $resolvedRoot -State "error" -Mode $mode -CurrentVersion $CurrentVersion -LatestVersion $latestVersionText -AssetName $assetName -AssetSizeBytes $assetSizeBytes -BytesDownloaded $assetSizeBytes -ReleaseUrl $releaseUrlText -Message "Downloaded package failed validation" -LastError $contractValidation.errorCode
    Remove-Item -Path $tempRoot -Recurse -Force -ErrorAction SilentlyContinue
    exit 0
  }

  Write-UpdateLog -LogPath $logPath -Message "Extracted package validation passed. packageVersion=$($contractValidation.packageVersion) manifestVersion=$($contractValidation.manifestVersion)"

  try {
    $planStats = Get-UpdatePlanStats -SourceRoot $sourceRoot -DestinationRoot $resolvedRoot
    $filesTotal = [int]$planStats.total
    $filesPlanned = [int]$planStats.planned
    Write-UpdateLog -LogPath $logPath -Message "Update file plan: total=$filesTotal planned=$filesPlanned"
  }
  catch {
    Write-UpdateError -LogPath $logPath -ErrorRecord $_ -Prefix "Failed to compute file plan"
  }
  ...
  if ($StageOnly) {
    ...
    Write-UpdateState -Root $resolvedRoot -State "staging" -Mode $mode -CurrentVersion $CurrentVersion -LatestVersion $latestVersionText -AssetName $assetName -AssetSizeBytes $assetSizeBytes -BytesDownloaded $assetSizeBytes -ReleaseUrl $releaseUrlText -Message "Staging update for next launch" -FilesTotal $filesTotal -FilesPlanned $filesPlanned -FilesProcessed 0 -FilesFailed 0
    ...
    if ($robocopyExitCode -gt 7) {
      ...
      Write-UpdateState -Root $resolvedRoot -State "error" -Mode $mode -CurrentVersion $CurrentVersion -LatestVersion $latestVersionText -AssetName $assetName -AssetSizeBytes $assetSizeBytes -BytesDownloaded $assetSizeBytes -ReleaseUrl $releaseUrlText -Message "Staging failed" -LastError "robocopy-$robocopyExitCode" -FilesTotal $filesTotal -FilesPlanned $filesPlanned -FilesProcessed 0 -FilesFailed $filesPlanned
      Remove-Item -Path $pendingDir -Recurse -Force -ErrorAction SilentlyContinue
      exit 0
    }
    ...
    $filesProcessed = if ($filesPlanned -ge 0) { $filesPlanned } else { 0 }
    Write-UpdateState -Root $resolvedRoot -State "staged" -Mode $mode -CurrentVersion $CurrentVersion -LatestVersion $latestVersionText -AssetName $assetName -AssetSizeBytes $assetSizeBytes -BytesDownloaded $assetSizeBytes -ReleaseUrl $releaseUrlText -Message "Update downloaded and staged for next launch" -FilesTotal $filesTotal -FilesPlanned $filesPlanned -FilesProcessed $filesProcessed -FilesFailed 0
    exit 0
  }
  ...
  if ($robocopyExitCode -gt 7) {
    ...
    Write-UpdateState -Root $resolvedRoot -State "error" -Mode $mode -CurrentVersion $CurrentVersion -LatestVersion $latestVersionText -AssetName $assetName -AssetSizeBytes $assetSizeBytes -BytesDownloaded $assetSizeBytes -ReleaseUrl $releaseUrlText -Message "Apply failed" -LastError "robocopy-$robocopyExitCode" -FilesTotal $filesTotal -FilesPlanned $filesPlanned -FilesProcessed 0 -FilesFailed $filesPlanned
    exit 0
  }

  Write-UpdateLog -LogPath $logPath -Message "Updated from $currentVersion to $latestVersion"
  $filesProcessed = if ($filesPlanned -ge 0) { $filesPlanned } else { 0 }
  Write-UpdateState -Root $resolvedRoot -State "updated" -Mode $mode -CurrentVersion $CurrentVersion -LatestVersion $latestVersionText -AssetName $assetName -AssetSizeBytes $assetSizeBytes -BytesDownloaded $assetSizeBytes -ReleaseUrl $releaseUrlText -Message "Update applied" -FilesTotal $filesTotal -FilesPlanned $filesPlanned -FilesProcessed $filesProcessed -FilesFailed 0
