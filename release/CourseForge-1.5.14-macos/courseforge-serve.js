...
const localTextbooksStatePath = path.join(packageRoot, "local-textbooks-state.json");
...
function getAutoUpdateScriptPath() {
  const candidates = [
    ...
    path.join(packageRoot, "autoupdate-courseforge.ps1"),
    path.join(packageRoot, "AutoUpdate-CourseForge.sh"),
    path.join(packageRoot, "autoupdate-courseforge.sh"),
  ];
...
function launchManualStageUpdate(checkPayload, options = {}) {
  ...
  const isPowerShellScript = updateScriptPath.toLowerCase().endsWith(".ps1");
  const command = isPowerShellScript ? "powershell.exe" : "bash";
  const spawnArgs = isPowerShellScript
    ? [
        "-NoProfile",
        "-ExecutionPolicy",
        "Bypass",
  ...
  if (typeof owner === "string" && owner.trim().length > 0) {
    if (isPowerShellScript) {
      spawnArgs.push("-Owner", owner.trim());
    } else {
      spawnArgs.push("--owner", owner.trim());
    }
  }
  ...
  if (typeof assetNameTemplate === "string" && assetNameTemplate.trim().length > 0) {
    if (isPowerShellScript) {
      spawnArgs.push("-AssetNameTemplate", assetNameTemplate.trim());
    } else {
      spawnArgs.push("--asset-name-template", assetNameTemplate.trim());
    }
  }
...
async function handleApiRoute(pathname, url, method, req, res) {
  ...
  if (pathname === "/api/local-textbooks-state") {
    if (method === "GET") {
      const payload = readJsonFile(localTextbooksStatePath);
    ...
    if (method === "POST") {
      let body;
      try {
        body = await readJsonRequestBody(req, 2 * 1024 * 1024);
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        const status = message === "request-body-too-large" ? 413 : 400;
        res.writeHead(status);
      ...
      const textbooks = Array.isArray(body?.textbooks) ? body.textbooks : [];
      writeJsonFile(localTextbooksStatePath, {
        updatedAt: new Date().toISOString(),
        textbooks,
      });
      res.writeHead(202);
      res.end(JSON.stringify({ ok: true, count: textbooks.length }));
      return;
    }
...
function startServer(finalPort) {
  ...
  const idleHeartbeatMonitor = setInterval(() => {
  ...
  if (typeof idleHeartbeatMonitor.unref === "function") {
    idleHeartbeatMonitor.unref();
  }
  ...
  server.on("error", (err) => {
    ...
    clearInterval(idleHeartbeatMonitor);
    process.exit(1);
  });
